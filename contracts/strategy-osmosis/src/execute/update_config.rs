...
pub fn execute_update_config(
    ...
    if let Some(quote_denom) = msg.quote_denom {
        config.quote_denom = quote_denom;
    }
    ...
    if let Some(deposit_denom) = msg.controller_deposit_denom {
        config.controller_deposit_denom = deposit_denom;
    }
    if let Some(transfer_channel_id) = msg.controller_transfer_channel_id {
        config.controller_transfer_channel_id = transfer_channel_id;
    }
    ...
    let resp = Response::new()
        ...
        .add_attribute("deposit_denom", config.controller_deposit_denom.to_string());
