use cosmwasm_std::{coins, Uint128, StdError, OverflowError};
...
fn unstake() {
    ...
    let err = execute_unstake(deps.as_mut(), info.funds[0].amount.clone(), info.sender).unwrap_err();
    ...
    let result = execute_unstake(deps.as_mut(), unstake_info.funds[0].amount.clone(), unstake_info.sender);
